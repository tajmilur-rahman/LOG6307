...
if cur.rowcount > 0 :
	...
	for row in rows :
		...
		cur.execute("select canonical from git_revision where commit='"+str(commit)+"' and (add+remove)>0");
			...
			l = [];
                        for _file in _files :
				#print "select count(*) from git_commit c, git_revision r where c.commit=r.commit and canonical='"+_file[0]+"' and c.author_dt < '"+str(author_date)+"' order by c.author_dt asc";
